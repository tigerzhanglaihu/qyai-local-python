...
# 获取检索问题的 QA 对象
def _get_retrieval_qa(vertor_db,VECTOR_DB_PATH,LLM_OLLAMA_MODEL,EMBEDDING_MODEL_PATH,EMBEDDING_MODEL):
...
def prompt_chat(prompt, user_question,LLM_OLLAMA_MODEL):
    try:
        # 定义提示模板
        prompt_template = PromptTemplate(
            input_variables=["user_question"],
            template= prompt + ": {user_question}"
        )
        #创建Langchain
        # 增加逻辑是否启用llama
        llm = Ollama(model=LLM_OLLAMA_MODEL)
